...
class DetectHanoiTower:
    ...
    def read_image_path(self, path) -> np.array:
        """
    ...
    def morphology_dilation(self, img: np.ndarray, kernel_size: tuple = (3,3), iterations: int = 1) -> np.ndarray:
        """
    ...
    def group_columns(self, columns: np.ndarray) -> list[np.ndarray]:
        """
    ...
    def cut_columns(self, columns: np.ndarray, img: np.ndarray, delta: int) -> dict:
        """
    ...
    def verificar_circulo(self, pt: np.ndarray, img_size: tuple) -> bool:
        """
    ...
    def detect_columns(self, edge_image: np.ndarray) -> np.ndarray:
        ...
        for contour, contour_hierarchy in zip(contours, hierarchy[0]):
            ...
            if contour_perimeter > 50:
                ...
                if contour_hierarchy[3] == -1:
                    ...
                    circle_coordinates = self.detect_circular_shapes(contour_mask, 100, 300, 12, 1)
                    if circle_coordinates is not None:  
                        ...
                        #self.show_image(contour_mask, "Contornos detectados (marcados)")
        ...
        raise ValueError("No se detectaron columnas en la imagen.")
    ...
    def detect_nested_circles(self, edged: np.ndarray, gray: np.ndarray, delta: int):
        ...
        for cnt in contours:
            ...
            if pe > 5:
                ...
                coordinates = self.detect_circular_shapes(black, 100, 300, 30, 1.5)
    ...
    def detect_square(self, edged: np.ndarray, blur: np.ndarray, delta: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
                ...
                if h[3] == -1 and h[2] != -1:
                    ...
                    if len(approx) == 4:
                        ...
                        if rblur.shape[0] > 0 and rblur.shape[1] > 0:
                            ...
                            canny_square_img = self.cannyfilter(rblur, l1=20, l2=60)
                            dilated_canny_squeare = self.morphology_dilation(canny_square_img, (3,3))
        ...
        # Se determina si se detectar al menos una columna
        if len(keys) > 0:
    ...
    def preprocess_image(self) -> None:
        ...
        self.a_norm = cv2.normalize(a, None, 0, 255, cv2.NORM_MINMAX)
        blur_img = self.blurfilter(self.gray_img, kernel_size=5)
        self.show_image(self.gray_img, 'Grises')
        self.blur_lab_img = self.blurfilter(self.a_norm, kernel_size=5)

        canny_img = self.cannyfilter(blur_img, l1=30, l2=40)

        self.dilated_img = self.morphology_dilation(canny_img, kernel_size=(3,3))
    ...
    def main(self, path) -> None:
        ...
        columns, square_blur, zoom = self.detect_square(self.dilated_img, self.blur_lab_img, delta=5)
...
if __name__ == '__main__':    
    ...
    image_directory = 'imagenes_5_dic_lab_2'
    #image_directory = 'cam'
    ...
    for image_file in image_files:
        image_file = 'img_rectangulos0_1.png'
